...
public class Deploys {
  ...
  private void initPomVersion() throws Throwable {
    ...
    LOGGER.info("pomVersion01={}", pomVersion);
    if (pomVersion != null) {
      return;
    }
    ...
    pomVersion = model.getProperties().getProperty("revision");
    LOGGER.info("pomVersion02={}", pomVersion);
    if (pomVersion != null) {
      return;
    }
    pomVersion = model.getVersion();
    LOGGER.info("pomVersion03={}", pomVersion);
    if (pomVersion != null) {
      return;
    }
    ...
    pomVersion = model.getProperties().getProperty("revision");
    LOGGER.info("pomVersion04={}", pomVersion);
    if (pomVersion != null) {
      return;
    }
    pomVersion = model.getVersion();
    LOGGER.info("pomVersion05={}", pomVersion);
    if (pomVersion != null) {
      return;
    }
  ...
  private boolean propertyVersion(Model model) {
    LOGGER.info("pomVersion02={}", pomVersion);
    if (pomVersion.startsWith("$")) {
      // ${revision}
      pomVersion = pomVersion.substring(2, pomVersion.length() - 1);
      pomVersion = model.getProperties().getProperty(pomVersion);
      LOGGER.info("pomVersion03={}", pomVersion);
      if (pomVersion != null) {
        return true;
      }
